...
// Salvamento automático habilitado como padrão
...
var appData = {
    ...
    'currentBoard': 0, // O índice do quadro atualmente aberto.
    'identifier': 0
};
...
Array.prototype.move = function (from, to) {
    /* Move um item do array para um índice específico do array. */
    this.splice(to, 0, this.splice(from, 1)[0]);
};

Array.prototype.insert = function (index, item) {
    /* Insere um item em um índice específico do array. */
...
function getMouseOverCard() {
    // A lista sobre a qual o cursor do mouse está atualmente.
    return document.querySelectorAll('.parent-card:hover')[0];
}

function getMouseOverItem() {
    // A tarefa sobre a qual o cursor do mouse está atualmente.
    return document.querySelectorAll('.parent-card > ul > li:hover')[0];
}

function getItemFromElement(element) {
    /* Obtém um objeto Item de um elemento de item de lista. */
...
function getCardFromElement(element) {
    /* Obtém um objeto Card de um elemento card div. */
    return currentCards().find(e => e.id === element.id);
}

/* Obtém um objeto de tabuleiro a partir de seu id exclusivo. */
function getBoardFromId(id) {

    return appData.boards.find(_b => _b.id === id);
}
/* Lista todos os quadros na barra lateral. */
function listBoards() {
...
function renderBoard(board) {
    ...
    document.title = 'Quadros | ' + currentBoard().name;
...
function toggleHoverStyle(show) {
    ...
    if (show) {
        ...
        _hoverStyle.innerHTML = ".parent-card:hover {background-color: #68696a;}.parent-card > ul > li:hover {background-color: #d1d1d1;}";
...
function addBoard() {
    ...
    if (!_boardTitle) return createAlert("Digite um nome para o quadro!");  // We don't create a board if it has no name.
    if (appData.boards.length >= 512) return createAlert("Limite máximo de quadros atingido.")  // or if there are already too many boards
...
class Card {
    ...
    renderItems() {
        ...
        for (let _item of this.items) {
            ...
            _newItemDeleteButton.addEventListener('click', () => {
                createConfirmDialog("Tem certeza de que deseja excluir esta tarefa?", () => this.removeItem(_item));
            });
    ...
    generateElement() {
        ...
        _newInput.placeholder = "Add Tarefa...";
        ...
        _newButton.addEventListener('click', () => {
            ...
            if (!_inputValue) return createAlert("Digite um nome para o cartão!");
...
class Board {
    ...
    addCard() {
        ...
        if (!_cardTitle) _cardTitle = `Lista ${this.cards.length + 1}`;
...
const cardContextMenu_clearCard = () => {
    createConfirmDialog('Tem certeza de limpar este cartão?', () => {
...
const cardContextMenu_deleteCard = () => {
    createConfirmDialog('Tem certeza de que deseja excluir este cartão?', () => {
        let _currentCardObject = getCardFromElement(cardContextMenu_currentCard);
...
const cardContextMenu_duplicateCard = () => {
    ...
    currentBoard().cards[_cIndex].name = _currentCardObject.name + ' Cópia';
...
function saveData() {
    window.localStorage.setItem('Listas-appData', JSON.stringify(appData));
}

function getDataFromLocalStorage() {
    return window.localStorage.getItem('Listas-appData');
}

function loadData() {
    let _data = window.localStorage.getItem('Listas-appData');
    if (_data) {
        ...
        let _defaultBoard = new Board("Quadro Principal", 'b0', { 'theme': null });
...
e_saveButton.addEventListener('click', () => { saveData(); createAlert("Dado salvo com sucesso!.") });

e_deleteButton.addEventListener('click', () => {
    createConfirmDialog('Tem certeza que deseja deletar este quadro?', () => {
        ...
        if (appData.boards.length === 0) {
            let _defaultBoard = new Board("Lista desconhecida", 'b0', { 'theme': null });
            appData.boards.push(_defaultBoard);
            appData.currentBoard = 0;
        }
        ...
        createAlert(`Quadro deletado "${_boardName}"`)
    });
});
